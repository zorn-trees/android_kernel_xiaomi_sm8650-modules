...
osif_cm_get_disconnect_reason(struct vdev_osif_priv *osif_priv, uint16_t reason)
{
	...
	// MIUI ADD:WIFI_LogEnhance
	if (reason == REASON_CLASS2_FRAME_FROM_NON_AUTH_STA || reason == REASON_4WAY_HANDSHAKE_TIMEOUT)
		osif_debug("[CON-TF-WIFI]wifi disconnect reason:%hu",reason);
	// END WIFI_LogEnhance
