...
int dsi_panel_mura_cmd_set(struct dsi_panel *panel){
	...
	if (panel == NULL)  return rc;
	if (!panel->mi_cfg.panel_mura_info_read_needed) return rc;
	...
	if (strcmp(batch, "00")) {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_MI_MURA_29COMMAND);
