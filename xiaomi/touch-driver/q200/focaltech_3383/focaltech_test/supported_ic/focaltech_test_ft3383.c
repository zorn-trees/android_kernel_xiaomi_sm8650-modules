...
static bool ft5572_rst_test(struct fts_test *tdata, bool *test_result)
{
	u8 value_1 = 0;
	u8 value_2 = 0;
	u8 value_3 = 0;
    ...
    if (ret < 0) {
        tmp_result = false;
        FTS_TEST_SAVE_ERR("failed to enter work mode,ret=%d\n", ret);
        goto out;
    }
#endif
    ...
    if (ret < 0) {
        FTS_TEST_SAVE_ERR("tp fw invaild");
        tmp_result = false;
        goto out;
    }

	ret = fts_test_read_reg(0xad, &value_1);
	if (ret < 0) {
		tmp_result = false;
	...
	ret = fts_test_write_reg(0xad, (value_1 + 1));
	if (ret < 0) {
		tmp_result = false;
	...
	ret = fts_test_read_reg(0xad, &value_2);
	if (ret < 0) {
		tmp_result = false;
        FTS_TEST_SAVE_ERR("value_2 read reg 0xad fail,ret=%d\n", ret);
        goto out;
    }

	if(value_2 != (value_1 + 1)) {
		tmp_result = false;
        FTS_TEST_SAVE_ERR("value_2 read(%x) not equal to write(%x)\n", value_2, (value_1 - 1));
        goto out;
	}
	...
	ret = fts_test_read_reg(0xad, &value_3);
	if (ret < 0) {
		tmp_result = false;
...
static int start_test_ft5572(void)
{
    ...
    /* rst test */
    if (true == test_item->rst_test) {
        ret = ft5572_rst_test(tdata, &temp_result);
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("rst_test fails");
        }
    }
    ...
    if (true == test_item->noise_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("noise_test fails");
        }
    }
    ...
    if (true == test_item->rawdata_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("rawdata_test fails");
        }
    }
    ...
    if (true == test_item->rawdata_uniformity_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("rawdata_uniformity_test fails");
        }
    }
    ...
    if (true == test_item->panel_differ_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("panel_differ_test fails");
        }
    }
    ...
    if (true == test_item->scap_rawdata_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("scap_rawdata_test fails");
        }
    }
    ...
    if (true == test_item->scap_cb_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("scap_cb_test fails");
        }
    }
    ...
    if (true == test_item->short_test) {
        ...
        if ((ret < 0) || (false == temp_result)) {
            test_result = false;
            FTS_TEST_ERROR("short_test fails");
        }
    }
...
static int fts_open_test(void)
{
    ...
    ret = fts_test_get_testparam_from_ini(ini_file_name);
    if (ret < 0) {
		FTS_TEST_SAVE_ERR("get testparam fails");
		return ret;
    }
	ret = fts_test_read_reg(0x14, &tdata->fre_num);
	if (ret < 0) {
...
static bool fts_rst_test(struct fts_test *tdata, bool *test_result)
{
	u8 value_1 = 0;
	u8 value_2 = 0;
	u8 value_3 = 0;
...
static int ft3383_get_rawdata(bool is_raw, int *data_buffer, int byte_num)
{
	int ret = 0;
	int i = 0;
	u8 data_type = 0;
...
static int ft3383_data_dump(int *rawdata, int *differ_data)
{
	int ret = 0;
	struct fts_test *tdata = fts_ftest;
	FTS_TEST_FUNC_ENTER();
	ret = ft3383_get_rawdata(true, rawdata, tdata->node.node_num * 2);
	if (ret) {
		FTS_TEST_ERROR("get rawdata error!");
		goto out;
	}
	ret = ft3383_get_rawdata(false, differ_data, tdata->node.node_num * 2);
	if (ret) {
...
struct test_funcs test_func_ft5572 = {
    ...
    .rst_test = fts_rst_test,
    .data_dump = ft3383_data_dump,
