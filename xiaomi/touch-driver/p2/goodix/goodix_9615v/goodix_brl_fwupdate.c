...
static int goodix_fw_update_thread(void *data)
{
	...
	if (fwu_ctrl->mode & UPDATE_MODE_SRC_REQUEST) {
		ts_debug("Firmware request update starts,[%s]", fwu_ctrl->fw_name);
...
int goodix_fw_update_init(struct goodix_ts_core *core_data)
{
	...
	snprintf(goodix_fw_update_ctrl.fw_name, sizeof(goodix_fw_update_ctrl.fw_name),
		"%s", core_data->board_data.fw_name);
