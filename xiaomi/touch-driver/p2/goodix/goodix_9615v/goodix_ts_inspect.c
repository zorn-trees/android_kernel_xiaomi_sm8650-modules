...
static int goodix_init_testlimits(struct goodix_ts_test *ts_test)
{
	...
	/* TODO: marseille don't test the following items */
	if (ts_core->bus->ic_type == IC_TYPE_MARSEILLE) {
	...
	if (ts_test->ts->bus->ic_type == IC_TYPE_MARSEILLE) {
		...
		if (status != 0xFD) {
			...
			for (i = 0; i < 6; i++) {
				...
				ts_test_read(ts_test, 0x2000, dbg_buf, sizeof(dbg_buf));
				ts_err("marseille short dbg_buf[0x%02x 0x%02x 0x%02x 0x%02x]",
