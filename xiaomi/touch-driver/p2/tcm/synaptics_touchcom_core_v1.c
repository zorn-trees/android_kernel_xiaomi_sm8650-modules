...
static int syna_tcm_v1_set_max_rw_size(struct tcm_dev *tcm_dev)
{
	...
	if (wr_size != tcm_dev->max_wr_size) {
		if (tcm_dev->max_wr_size == 0)
			...
			tcm_dev->max_wr_size =
				MIN_DATA(wr_size, tcm_dev->max_wr_size);
		...
		unsigned char *data, unsigned int size, unsigned int data_len)
{
	...
	retval = syna_pal_mem_cpy((unsigned char *)id_info,
			...
			data,
			size,
			MIN_DATA(sizeof(*id_info), data_len));
	if (retval < 0) {
		...
		unsigned char *status_report_code)
{
	...
	if (tcm_msg->payload_length > PAYLOAD_MAX_LENGTH) {
		...
		LOGE("receive pending code, payload_length = %d, tcm_read_max_length = %d, read count = %d, try to clear ic spi buffer\n",
...
do_dispatch:
	...
	if (tcm_msg->predict_reads && do_predict) {
		if (tcm_dev->max_rd_size == 0)
			...
			tcm_msg->predict_length = MIN_DATA(tcm_msg->payload_length,
...
/**
 * syna_tcm_v1_write_message_timeout()
 *
 * Write message including command and its payload to TouchCom device.
 * Then, the response of the command generated by the device will be
 * read in and stored in internal buffer.resp.
 ...
 *                         a positive value presents the time for polling;
 *                         or, set '0' or 'RESP_IN_ATTN' for ATTN driven
 *    [ in] timeout_ms_resp: timeout time for response reading.
 *                         set positive value for timeout
 *
 *
 * @return
 *    0 or positive value on success; otherwise, on error.
 */
static int syna_tcm_v1_write_message_timeout(struct tcm_dev *tcm_dev,
	unsigned char command, unsigned char *payload,
	unsigned int length_total, unsigned int length,
	unsigned char *resp_code, unsigned int delay_ms_resp, unsigned int timeout_ms_resp)
{
	int retval = 0;
	unsigned int idx;
	unsigned int chunks;
	...
	if (timeout_ms_resp <= 0) {
		timeout_ms_resp = CMD_RESPONSE_TIMEOUT_MS;
	}
...
void syna_tcm_v1_set_ops(struct tcm_dev *tcm_dev)
{
	...
	tcm_dev->write_message_timeout = syna_tcm_v1_write_message_timeout;
