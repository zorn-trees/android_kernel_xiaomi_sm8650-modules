...
static int fts_parse_dt(struct device *dev, struct fts_ts_platform_data *pdata)
{
    ...
    ret = of_property_read_u32(np, "focaltech,max-x",
			       &pdata->x_resolution);
    if (ret < 0)
	    return dev_err_probe(dev, ret, "missing focaltech,max-x\n");

    ret = of_property_read_u32(np, "focaltech,max-y",
			       &pdata->y_resolution);
    if (ret < 0)
	    return dev_err_probe(dev, ret, "missing focaltech,max-y\n");

    ret = of_property_read_u32(np, "focaltech,rx-num", &pdata->rx_num);
    if (ret < 0)
	    return dev_err_probe(dev, ret, "missing focaltech,rx-num\n");

    ret = of_property_read_u32(np, "focaltech,tx-num", &pdata->tx_num);
    if (ret < 0)
	    return dev_err_probe(dev, ret, "missing focaltech,tx-num\n");
...
{
	return fts_data->pdata->x_resolution;
}

int fts_get_y_resolution(void)
{
	return fts_data->pdata->y_resolution;
}

int fts_get_rx_num(void)
{
	return fts_data->pdata->rx_num;
}

int fts_get_tx_num(void)
{
	return fts_data->pdata->tx_num;
}

u8 fts_get_super_resolution_factor(void) 
{
	return (u8)fts_data->pdata->super_resolution_factors;
}
...
void fts_init_hardware_param(void)
{
	...
	memcpy(hardware_param.config_file_name, "zorn_thp_config.ini",
	       strlen("zorn_thp_config.ini"));
...
void fts_init_xiaomi_touchfeature_v3(struct fts_ts_data *ts_data)
{
	...
	/*
	 * Zorn's FT5672 factory-test implementation has not been published.
	 * Do not expose the FT3383 register test inherited from the reference.
	 */
	hardware_operation.ic_self_test = NULL;
	...
	xiaomi_register_panel_notifier(fts_data->dev, TOUCH_ID,
